...
TEST_CASE("Operator +, -, +=, -=") {
    ...
    SquareMat c = a + b;
    CHECK(c[0][0] == doctest::Approx(4));
    CHECK(c[1][1] == doctest::Approx(6));
    c -= b;
    CHECK(c[0][0] == doctest::Approx(1));
    CHECK(c[1][1] == doctest::Approx(2));
    c += b;
    CHECK(c[0][0] == doctest::Approx(4));
    CHECK(c[1][1] == doctest::Approx(6));
}
...
TEST_CASE("Unary minus and scalar multiplication/division") {
    ...
    SquareMat neg = -a;
    CHECK(neg[0][0] == doctest::Approx(-2));
    CHECK(neg[1][1] == doctest::Approx(4));
    a *= 3;
    CHECK(a[0][0] == doctest::Approx(6));
    CHECK(a[1][1] == doctest::Approx(-12));
    a /= 3;
...
TEST_CASE("Matrix multiplication and element-wise multiplication") {
    ...
    CHECK(elem[1][1] == doctest::Approx(8));
}
...
TEST_CASE("Modulo operator with scalar and %= operator") {
    ...
    SquareMat mod = a % 4;
    CHECK(mod[0][0] == doctest::Approx(1));
    CHECK(mod[1][1] == doctest::Approx(1));
    a %= 4;
...
TEST_CASE("Transpose, Power, and Determinant") {
    ...
    a.setSingleValue(1,1,4);
    SquareMat trans = ~a;
    CHECK(trans[0][1] == doctest::Approx(3));
    CHECK(trans[1][0] == doctest::Approx(2));
    SquareMat id = a ^ 0;
    CHECK(id[0][0] == doctest::Approx(1));
    CHECK(id[1][1] == doctest::Approx(1));
    SquareMat pow = a ^ 2;
    CHECK(pow[0][0] == doctest::Approx(7));
    CHECK(pow[1][1] == doctest::Approx(22));
